...
module.exports = function(dpkg, resolvedInputs, emitter){
  ...
  var priors = resolvedInputs.map(function(x){return x;});

  //patch priorSchema as name is not here due to cache
  priorSchema.required = priorSchema.required.filter(function(x){return x!== 'name';});
  ...
  model.observations.forEach(function(obs){
    if( (obs.distribution != 'discretized_normal') && (obs.distribution != 'binomial')){
      throw new Error("For the moment, SSM only supports 'discretized_normal' and 'binomial' distributions for observations.");
    }
  });
  ...
  var estimatedNames = priors
    ...
    .map(function(x){ return x.require.resource });
