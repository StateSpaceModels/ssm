...
function makeLhs(dpkgRoot, dpkg, options, callback){
  ...
  links.resolve(dpkgRoot, dpkg, dpkg.model.inputs, function(err, inputs){
    ...
    var pfit = inputs
      .filter(function(x){
        return ('require' in x) && !('fields' in x.require) && ('data' in x) && ('distribution' in x.data) && (x.data.distribution !== 'fixed');       
      })
...
function _qify(input, p){
  ...
  if ( !( ('require' in input) && !('fields' in input.require) && ('data' in input) && ('distribution' in input.data) ) ){
    return undefined;
  }
...
function makeFinputs(inputs, t0, options){
  ...
  inputs.forEach(function(input){
    ...
    if(('require' in input) && !('fields' in input.require) && ('data' in input)){
    ...
    } else if(('require' in input) && ('fields' in input.require)) {
